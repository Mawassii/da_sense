...
else
{
	...
	function checkCredentials($username, $password, $right)
	{
		return true;
		
		$mysqli_user = new mysqli(UM_HOST, UM_LGN, UM_PWD, UM_DB);
		
		$unprep_stmt = "SELECT count(*)
						FROM user
						JOIN mapping_user_right ON (user.ID = mapping_user_right.user)
						JOIN rights ON (mapping_user_right.rights = rights.ID)
						WHERE user.username = ?
						AND user.password = ?
						AND rights.name = ?";

		$stmt = $mysqli_user->prepare($unprep_stmt);
		$stmt->bind_param('sss', $username, md5($password), $right); // bind parameters
	...
	*/
	...
	class DataValue
	{
		...
		public function __construct($type, $value, $ts, $samples, $loc)
		{
			$this->type = $type;
			$this->value = $value;
			$this->timestamp = $ts;
	...
	class DASense
	{
		 ...
		 */
		function getData($typesConfig, $timeConfig, $locationConfig, $credentials)
		{ 
			//check if all parameters exist
			if($typesConfig == null || $timeConfig == null || $locationConfig == null || $credentials == null) throw new SoapFault("ERROR", "Not all segments are specified.");
			
			if(!checkCredentials($credentials->user,$credentials->password,"ws_read"))
				throw new SoapFault("ERROR", "Credentials are not correct.");
		 ...
		 */
		function getDataForNode($nodeID, $credentials)
		{ 
			if(!checkCredentials($credentials->user,$credentials->password,"ws_read"))
				throw new SoapFault("ERROR", "Credentials are not correct.");
		 ...
		 */
		function setData($node, $credentials)
		{ 
			if(!checkCredentials($credentials->user,$credentials->password,"ws_write"))
				throw new SoapFault("ERROR", "Credentials are not correct.");
		 ...
		 * Return: Array( Nodes )
		 */
		function getNodes($typesConfig, $credentials)
		{ 
			if(!checkCredentials($credentials->user,$credentials->password,"ws_read"))
				throw new SoapFault("ERROR", "Credentials are not correct.");
		 ...
		 */
		function getDataTypeInfos($credentials)
		{
			if(!checkCredentials($credentials->user,$credentials->password,"admin_read"))
				throw new SoapFault("ERROR", "Credentials are not correct.");
